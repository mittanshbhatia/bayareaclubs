-- Full club idea workflow enhancements (replaces partially failed 20260906010000)
...
create table if not exists public.club_idea_versions (
...
create index if not exists club_idea_versions_idea_created_idx
  on public.club_idea_versions (idea_id, version_number desc);

alter table public.club_idea_versions enable row level security;

drop policy if exists idea_versions_authorized_select on public.club_idea_versions;
drop policy if exists idea_versions_no_insert on public.club_idea_versions;
drop policy if exists idea_versions_no_update on public.club_idea_versions;
drop policy if exists idea_versions_no_delete on public.club_idea_versions;
...
drop trigger if exists club_ideas_freeze_version on public.club_ideas;
...
drop trigger if exists club_ideas_notify_status on public.club_ideas;
...
drop trigger if exists club_idea_versions_immutable on public.club_idea_versions;
